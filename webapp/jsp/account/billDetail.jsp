...
$(document).ready(function(){
	...
	$("#mySearch").bind("click",function(){
		...
		location.href="${_base}/accountInfoAction/toBillDetail?softName="+encodeURI(encodeURI(cond_softName))+"&startDate="+cond_startDate+"&endDate="+cond_endDate;
	});
});
...
(function($) {
	$.extend({
				"easypage" : function(options) {
					...
					//拼接导航按钮
					var navigatehtml = "<ul><li id=pagepre><A href='${_base}/accountInfoAction/toBillDetail?page=<%=abvpage %>&softName="+encodeURI(encodeURI(softName))+"&startDate="+startDate+"&endDate="+endDate+"'>上一页</A></li>";
					for (var i = 1; i <= pagecount; i++) {
						if(currentpage == i){
							navigatehtml += '<li class="x_shuz"><A href="${_base}/accountInfoAction/toBillDetail?page='+i+'&softName='+encodeURI(encodeURI(softName))+'&startDate='+startDate+'&endDate='+endDate+'">'
								+ i + '</A></li>';
						}else{
							navigatehtml += '<li class="pagenavigate"><A href="${_base}/accountInfoAction/toBillDetail?page='+i+'&softName='+encodeURI(encodeURI(softName))+'&startDate='+startDate+'&endDate='+endDate+'">'
								+ i + '</A></li>';
						}
					...
					navigatehtml += "<li id='pagenext'><A href='${_base}/accountInfoAction/toBillDetail?page=<%=nextpage %>&encodeURI(encodeURI(softName))="+softName+"&startDate="+startDate+"&endDate="+endDate+"'>下一页</A></li>"+
			          "<li>共<%=totalpage%>页</li>"+
			          "<li>到 第</li>"+
			          "<li><input name='x_f_input' type='text' id='x_f_input' class='x_f_input' onkeyup='checkPageNo()' style='text-align:center'/></li>"+
			          "<li>页</li>"+
			          "<li class='x_tzh'><A href='${_base}/accountInfoAction/toBillDetail?softName="+encodeURI(encodeURI(softName))+"&startDate="+startDate+"&endDate="+endDate+"&page=' onclick=this.href=this.href+document.getElementById('x_f_input').value>跳转</A></li>"+
			          "</ul>";
...
<div class="wai_da">
 ...
 <p><input name="" type="text"  value="${softName }" id="softName" class="ej_input"/></p>
 
 
 <p class="ej_wz">购买时间：</p>
  <p><input name="" value="${startDate }" type="text"  id="startDate" onClick="WdatePicker({maxDate:'#F{$dp.$D(\'endDate\')}','dateFmt':'yyyyMM'});"  class="ej_input_dua"/></p>
						<p class="x_g">-</p>
						<p><input name="" value="${endDate }" type="text"  id="endDate" onClick="WdatePicker({minDate:'#F{$dp.$D(\'startDate\')}','dateFmt':'yyyyMM'});" class="ej_input_dua"/></p>
...
<table width="100%" border="1">
  ...
  <c:forEach items="${billList}"  var="billDate">
  <tr>
  	<td align="center" valign="middle">${billDate.softwareName }</td>
    <td align="center" valign="middle">${billDate.cycleMonth }</td>    
    <td align="center" valign="middle">${billDate.apportionAmount }</td>
  </tr>
  </c:forEach>
</table>
 </div>
